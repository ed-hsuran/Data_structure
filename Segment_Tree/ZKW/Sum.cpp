...
void modify(int i, int v){ //單點修改
	for (zkw[i+=n] = v; i>1; i>>=1) zkw[i>>1] = zkw[i] + zkw[i^1];
} // ^互斥或閘 --> 此用於偶數++ 奇數--

ll query(int l, int r){ //詢問區域總和
	ll ans = 0;
	for (l+=n, r+=n; l<r; l>>=1, r>>=1){
		if (l & 1) ans = ans + zkw[l++]; //如果 l 是右子樹則右移
		if (r & 1) ans = ans + zkw[--r]; //如果r-1是左子樹則左移
	}
	return ans;
} // &閘 --> 此用於判定奇偶(左右子樹)
...
int main(){
	...
	modify(6, 10);
	show();
	cout << "第0個到第4個的總和? " << query(0, 5) << "\n";
	cout << "第3個到第7個的總和? " << query(3, 8) << "\n";
}
